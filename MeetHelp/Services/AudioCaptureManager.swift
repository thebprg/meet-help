...
class AudioCaptureManager: NSObject, ObservableObject {
    ...
    func startCapture(audioHandler: @escaping (Data) -> Void) async {
        self.audioCallback = audioHandler
        
        // First check permission
        guard await checkPermission() else {
            self.error = "Screen Recording permission not granted. Go to System Settings > Privacy & Security > Screen Recording and enable for Terminal (or your IDE)."
        ...
        do {
            ...
            // Create a filter to capture the display
            ...
            // Exclude app audio to avoid feedback
            configuration.excludesCurrentProcessAudio = true
            
            // We don't need video, minimize its impact
            configuration.width = 2
            configuration.height = 2
            configuration.minimumFrameInterval = CMTime(value: 1, timescale: 1) // 1 FPS
            configuration.showsCursor = false
            
            // Create the stream with a delegate for error handling
            ...
            // Common error codes
            if error.code == 1003 {
                self.error = "Screen Recording permission denied. Grant access in System Settings > Privacy & Security > Screen Recording, then restart the app."
            }
        }
    }
